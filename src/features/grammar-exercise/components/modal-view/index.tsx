...
export function ModalView({
  ...
  const {
    ...
    error,
    refetch,
  } = useGetGrammarExerciseByIdQuery(selectedId || EMPTY.str, false);
  ...
  useEffect(() => {
    ...
    if (isAddMode) {
      ...
      const exercise = data.data.grammarExercise;
      form.reset({
        type: exercise?.type ?? "MULTIPLE_CHOICE",
        question: exercise?.question ?? EMPTY.str,
        answer: exercise?.answer ?? EMPTY.str,
        options: exercise?.options ?? { options: [EMPTY.str] },
        explanation: exercise?.explanation ?? EMPTY.str,
        order: exercise?.order ?? 1,
        score: exercise?.score ?? 10,
        status: exercise?.status ?? true,
        grammarTopicId: exercise?.grammarTopicId ?? EMPTY.str,
      });
    }
        ...
        {isLoading ? (
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <FormFields form={form} isViewMode={isViewMode} />
